...
%include fusion-linux-14-base.ks
...
#gnome-do
synapse # Gnome-do alternative, 2.7 M
...
Miro # InternetTV video player, 16 M
#gnome-mplayer
...
#giver # easy local file sharing, based on Mono 0.3 M
...
#geeqie # Image viewer and browser, 6 M
#gloobus-preview # mac like image preview
...
pan
tucan
uget # Gnome download manager, 0.4 M
#chromium
#google-chrome-stable
chromium-browser
thunderbird # Mail client, 27 M
#midori
#epiphany
#evolution-mapi
-evolution* # Evolution mail client, 40 M
...
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /desktop/gnome/applications/browser/exec /usr/lib/chromium-browser/chromium-wrapper > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /desktop/gnome/url-handlers/unknown/command "/usr/lib/chromium-browser/chromium-wrapper %s" > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /desktop/gnome/url-handlers/http/command "/usr/lib/chromium-browser/chromium-wrapper %s" > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /desktop/gnome/url-handlers/https/command "/usr/lib/chromium-browser/chromium-wrapper %s" > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /desktop/gnome/url-handlers/about/command "/usr/lib/chromium-browser/chromium-wrapper %s" > /dev/null
