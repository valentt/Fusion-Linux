...
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string file:///desktop/gnome/application/browser/exec Alliance-fusion-fancy > /dev/null

# make Chromium default browser
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t string /apps/metacity/general/theme /usr/bin/chromium-browser > /dev/null

#fix mintMenu small icons bug
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t int file:///apps/mintMenu/plugins/applications/favicon_size 32 > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t int file:///apps/mintMenu/plugins/applications/icon_size 16 > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t int file:///apps/mintMenu/plugins/system-management/icon_size 22 > /dev/null
gconftool-2 --direct --config-source=xml:readwrite:/etc/gconf/gconf.xml.defaults -s -t int file:///apps/mintMenu/plugins/applications/icon_size 22 > /dev/null
